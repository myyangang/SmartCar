...
void USART2_IRQHandler(void)
{
    if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)
    {
        ...
        {
            wireless_uart_callback_ch1();
        }
        else if(wireless_type == BULETOOTH_CH9141)
        {
...
void USART3_IRQHandler(void)
{
    if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)
    {
        ...
        switch(data) {
            case 'a':
                motorRight.pid.pCoef += 1;
                break;
            case 'b':
                if(motorRight.pid.pCoef >= 1){
                    motorRight.pid.pCoef -= 1;
                }
                break;
            case 'c':
                ...
                break;
            case 'e':
                motorRight.pid.iCoef += 1;
                break;
            case 'f':
                if(motorRight.pid.iCoef >= 1){
                    motorRight.pid.iCoef -= 1;
                }
                break;
            case 'g':
                ...
                break;
            case 'i':
                motorRight.pid.dCoef += 1;
                break;
            case 'j':
                if(motorRight.pid.dCoef >= 1){
                    motorRight.pid.dCoef -= 1;
                }
                break;
            case 'k':
                ...
                break;
            case 'm':
                motorLeft.pid.target += 50;
                motorRight.pid.target += 50;
                break;
            case 'n':
                if(motorLeft.pid.target >= 50){
                    motorLeft.pid.target -= 50;
                    motorRight.pid.target -= 50;
                }
                break;
            case 'o':
                ...
                break;
            case 'p':
                motorLeft.pid.target = -motorLeft.pid.target;
                motorRight.pid.target = -motorRight.pid.target;
                break;
            case 'q':
                motorLeft.pid.target = 100;
                motorRight.pid.target = 100;
                break;
            case 'r':
                motorLeft.pid.target = 700;
                motorRight.pid.target = 700;
                break;
            case '2':
                servo.pwm += 100;
                break;
...
void TIM1_UP_IRQHandler(void)
{
    if(TIM_GetITStatus(TIM1, TIM_IT_Update) != RESET)
    {
        ...
        int16_t measurementLeft,measurementRight;
        measurementLeft = -encoder_get_count(TIM4_ENCOEDER);
        encoder_clean_count(TIM4_ENCOEDER);
        measurementRight = -encoder_get_count(TIM9_ENCOEDER);
        encoder_clean_count(TIM9_ENCOEDER);

        motorLeft.pid.measurement = measurementLeft;
        motorRight.pid.measurement = measurementRight;
...
//                motorLeft.pwm,motorLeft.pid.deltaOutput, motor.direction ? 1000: 0, 0,
                motorRight.pid.target, motorLeft.pid.target, measurementLeft, measurementRight,
                motorRight.pid.pCoef, motorRight.pid.iCoef, motorRight.pid.dCoef, 0
        );
    }
}

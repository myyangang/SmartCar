...
void uart_gpio_init(uart_pin_enum tx_pin, uart_pin_enum rx_pin)
{
    ...
    else if(tx_pin == UART6_TX_E10 )    gpio_init(E10,  GPO, 0, GPO_AF_PUSH_PULL);
...
//-------------------------------------------------------------------------------------------------------------------
void uart_write_string(uart_index_enum uartn, const int8_t *str)
{
    while(*str)
    {
...
//  @param      *dat            接收数据的地址
//  @return     void
//  Sample usage:               uint8_t dat; uart_getchar(USART_1,&dat);       // 接收串口1数据  存在在dat变量里
//-------------------------------------------------------------------------------------------------------------------
...
//  Sample usage:               uint8_t dat; uart_query(USART_1,&dat);       // 接收串口1数据  存在在dat变量里
//-------------------------------------------------------------------------------------------------------------------

...
void __updateMotorRotation(Motor *motor){ // TODO:左右电机分开控制
    pwm_set_duty(MOTOR_L_PWM_CHANNEL, (motor->direction) ? (10000 - motor->pwm) : motor->pwm);
    pwm_set_duty(MOTOR_R_PWM_CHANNEL, (motor->direction) ? (10000 - motor->pwm) : motor->pwm);
...
void changeMotorRotation(Motor *motor, MathOperation operation, uint32_t offset){
    ...
    if(operation == PLUS){ // 如果要给正向加速度
        ...
        motor->pwm = max(offset, PWM_DUTY_MAX); // TODO:为什么PWM最大值改成了9000？
